...
3.1 Ahora analizaremos las variables más importantes según el color del vino o tipo
...
library(car)
leveneTest(total.sulfur.dioxide ~ color, data = totalData[,c(1,2,3,4,5,6,7,9,10,13,14,15)])

```

Como el p-valor es menor que 0.05 se rechaza la hipotesis nula de homocedasticidad y se concluye que la variable total.sulful.dioxide presenta varianzas estadisticamente diferentes para los dos tipos de vino.
...
3.4 Ahora analizaremos la correlación entre las diferentes variables
